...
module JuiceC {
    ...
    export class Control {
        ...
        symbolTableElement: HTMLTableElement;
        verboseElement: HTMLInputElement;
        clipboard;
        ...
        static init(): void {
            ...
            _Control.symbolTableElement = (<HTMLTableElement>document.getElementById("symbolTable"));
            _Control.verboseElement = (<HTMLInputElement>document.getElementById("verboseCheck"));
            ...
            // Clear symbol table
            const rowCount: number = _Control.symbolTableElement.rows.length;
            for (let i = rowCount - 1; i > 1; i--) {
                _Control.symbolTableElement.deleteRow(i);
            }
            ...
            if ((<HTMLInputElement>document.getElementById("sourceCode")).value.length == 0) {
                ...
                for (let programIndex = 0; programIndex < _Control.lexResults.length; programIndex++) {
                    ...
                    // If no tokens are found, output an error
                        ...
                        _Control.putMessage(INFO + "\tCompilation failed due to no tokens"
                            + " being found. Where's the code?");
                        ...
                        const parseResult: ParseResult = _Parser.parse(_Control.lexResults[programIndex].tokens);
                        if (_Control.verboseElement.checked) {
                        ...
                        if (!parseResult.error) {
                            const cst = parseResult.cst.traverseTreeCST(_Control.treantCST, (programIndex + 1));
                            ...
                            _Control.putMessage(INFO + "\tCST failed to generate "
                                + "due to parsing errors");
                        }
                        _Control.putMessage(INFO + "\tParsing complete with "
                             + parseResult.errors + " ERROR(S)");

                        // Semantic analysis only if there were no parser errors
                        if (!parseResult.error) {
                            const _Semantic: Semantic = new Semantic(parseResult.cst);
                            _Control.putMessage(INFO + "\tStarting Semantic Analysis of Program "
                                 + (programIndex + 1));
                            const semanticResult = _Semantic.analyze();
                            ...
                            if (semanticResult.errors === 0) {
                                if (_Control.verboseElement.checked) {
                                    const ast = semanticResult.ast.traverseTreeAST(_Control.treantAST, (programIndex + 1));
                                    ...
                                    for (let l = 0; l < symbols.length; l++) {
                                        const row: HTMLTableRowElement = _Control.symbolTableElement.insertRow(-1);
                                        const program: HTMLTableCellElement = row.insertCell(0);
                                        program.innerHTML = (programIndex + 1).toString();
                                        const key: HTMLTableCellElement = row.insertCell(1);
                                        key.innerHTML = symbols[l].key;
                                        const type: HTMLTableCellElement = row.insertCell(2);
                                        type.innerHTML = symbols[l].type;
                                        const scope: HTMLTableCellElement = row.insertCell(3);
                                        scope.innerHTML = symbols[l].scope;
                                        const lineNum: HTMLTableCellElement = row.insertCell(4);
                                        lineNum.innerHTML = symbols[l].line;
                                        const colNum: HTMLTableCellElement = row.insertCell(5);
                                        colNum.innerHTML = symbols[l].col;
                                    }

                                    const scopeTree = semanticResult.ast.traverseTreeAST(_Control.treantAST, (programIndex + 1));
                                    _Control.scopeTreeElement.value += "Program "
                                         + (programIndex + 1) + "\n";
                                ...
                                _Control.putMessage(INFO + "\tSemantic Analysis complete with "
                                     + semanticResult.errors + " ERROR(S) and "
                                     + semanticResult.warnings + " WARNING(S)");
                                _CodeGen = new CodeGen(semanticResult);
                                _Control.putMessage(INFO + "\tStarting Code Generation of Program "
                                     + (programIndex + 1));
                                ...
                                _Control.putMessage(INFO + "\tCode Generation complete with "
                                     + _CodeGen.errors + " ERROR(S)");
                                _Control.scrollToBottom();
                            } else {
                                _Control.putMessage(INFO + "\tAST failed to generate due to "
         ...
         * @param programIndex 
         */
        prepareLexLog(programIndex: number): void {
         ...
         * @param programIndex 
         */
        lexerLog(lexResults: Array<LexResult>, programIndex: number): void {
            if (_Control.verboseElement.checked) {
                // Print all tokens
                for (let i = 0; i < lexResults[programIndex].tokens.length; i++) {
                    _Control.putMessage(DEBUG + " - " + LEXER + " - "
                        + lexResults[programIndex].tokens[i].type + " [ "
                        + lexResults[programIndex].tokens[i].value + " ] found at ("
                        + lexResults[programIndex].tokens[i].lineNum + ":"
                        + lexResults[programIndex].tokens[i].colNum + ")");
                }
            }
            // Print all warnings
            if (_Control.lexResults[programIndex].warnings.length > 0) {
                for (let i = 0; i < lexResults[programIndex].warnings.length; i++) {
                    ...
                    if (lexResults[programIndex].warnings[i].warningType === WarningType.NoEOP) {
                        _Control.putMessage(DEBUG + " - " + LEXER + " - " + WARNING + ": "
                            + WarningType.NoEOP + " [ $ ] detected at end-of-file. Adding to"
                            + " end-of-file for you.");
                        ...
                        _Control.putMessage(DEBUG + " - " + LEXER + " - " + WARNING
                             + ": Not really sure why I'm warning so oops?");
                    }
                }
            }
            // Print all errors
            for (let i = 0; i < lexResults[programIndex].errors.length; i++) {
                switch (lexResults[programIndex].errors[i].errorType) {
                    ...
                    case ErrorType.InvalidT: {
                        _Control.putMessage(DEBUG + " - " + LEXER + " - " + ERROR + ": "
                             + ErrorType.InvalidT + " [ " + lexResults[programIndex].errors[i].value
                    ...
                    case ErrorType.NoEndComment: {
                        _Control.putMessage(DEBUG + " - " + LEXER + " - " + ERROR + ": "
                             + ErrorType.NoEndComment + " brace (*/) for comment starting at [ "
                             + lexResults[programIndex].errors[i].value + " ] found at ( "
                             + lexResults[programIndex].errors[i].lineNum + ":"
                             + lexResults[programIndex].errors[i].colNum + " )");
                        break;
                    }
                    ...
                    case ErrorType.InvalidNewLine: {
                        _Control.putMessage(DEBUG + " - " + LEXER + " - " + ERROR + ": "
                             + ErrorType.InvalidNewLine + " [ \\n ] found at ( "
                             + lexResults[programIndex].errors[i].lineNum + ":"
                             + lexResults[programIndex].errors[i].colNum
                             + " ) - New lines are not allowed in comments or strings");
                        break;
                    }
                    ...
                    default: {
                        _Control.putMessage(DEBUG + " - " + Lexer + " - " + ERROR
                             + ": Unknown error found [ " + lexResults[programIndex].errors[i].value
                             + " ] at ( " + lexResults[programIndex].errors[i].lineNum + ":"
                             + lexResults[programIndex].errors[i].colNum + " )");
                        break;
                    }
                }
            }
            _Control.putMessage(INFO + "\tLexical Analysis complete with "
                 + lexResults[programIndex].warnings.length + " " + WARNING + "(S) and "
                 + lexResults[programIndex].errors.length + " " + ERROR + "(S)");
        }
