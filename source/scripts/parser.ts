...
module JuiceC {
	...
	export class Parser {
         ...
         */
        public parse(tokens: Array<Token>): ParseResult {
            this.init(tokens);
        ...
        public parseBlock(production: Array<Production>, expected: boolean): boolean {
            if (this.matchAndConsumeToken(TokenType.LBrace, production, Production.Block, false) && this.parseStatementList() 
                ...
                if (expected) {
                    this.log.push(DEBUG + " - " + PARSER + " - ERROR: " + ErrorType.BlockExpected + " - found [ " 
        ...
        public parseStatementList(): boolean {
            ...
            // Empty string is acceptable
                ...
                this.log.push(DEBUG + " - " + PARSER + " - VALID: " + Production.Epsilon + " found at ( " 
        ...
        public parsePrintStatement(production: Array<Production>): boolean {
            if (this.matchAndConsumeToken(TokenType.Print, production, Production.PrintStatement, false) 
                && this.matchAndConsumeToken(TokenType.LParen, null, null, true) && this.parseExpr([Production.Expr]) 
         ...
         * @param production the productions being rewritten
         */
        public parseVarDecl(production: Array<Production>): boolean {
            if (this.parseType(production.concat([Production.VarDecl])) && this.parseId(null, true)) {
        ...
        public parseWhileStatement(production: Array<Production>): boolean {
            if (this.matchAndConsumeToken(TokenType.While, production, Production.WhileStatement, false) 
        ...
        public parseExpr(production: Array<Production>): boolean {
            if (this.parseIntExpr(production) || this.parseStringExpr(production) || this.parseBoolExpr(production, false) 
                ...
                this.log.push(DEBUG + " - " + PARSER + " - ERROR: " + ErrorType.ExprExpected + " - found [ " 
                    + this.tokens[this.currentTokenIndex].type + ", " +  this.tokens[this.currentTokenIndex].value + " ] at ( " 
                    + this.tokens[this.currentTokenIndex].lineNum + ":" + this.tokens[this.currentTokenIndex].colNum 
                    + " ) - " + Production.Expr + " ::== " + Production.IntExpr + " or " + Production.StringExpr + " or " 
        ...
        public parseStringExpr(production: Array<Production>): boolean {
            if (this.matchAndConsumeToken(TokenType.Quote, production, Production.StringExpr, false) 
        ...
        public parseBoolExpr(production: Array<Production>, expected: boolean): boolean {
            if (this.parseBoolVal(production)) {
                ...
                this.cst.ascendTree();
                return true;
            } else if (this.matchAndConsumeToken(TokenType.LParen, production, Production.BooleanExpr, false) 
                && this.parseExpr([Production.Expr]) && this.parseBoolOp(null) && this.parseExpr([Production.Expr]) 
                && this.matchAndConsumeToken(TokenType.RParen, null, null, true)) {
                    this.cst.ascendTree();
                        return true;
            }
            if (expected) {
                this.log.push(DEBUG + " - " + PARSER + " - ERROR: " + ErrorType.BoolExprExpected + " - found [ " 
                    ...
                    + this.tokens[this.currentTokenIndex].colNum + " ) - " + Production.Expr + " ::== ( " + Production.Expr 
                    + " " + Production.BoolOp + " " + Production.Expr + " )");
                this.error = true;
                this.errors++;
            }
            return false;
        ...
        public parseId(production: Array<Production>, expected: boolean): boolean {
            ...
            if (expected) {
                this.log.push(DEBUG + " - " + PARSER + " - ERROR: " + ErrorType.IdExpected + " - found [ " 
                    + this.tokens[this.currentTokenIndex].type + ", " + this.tokens[this.currentTokenIndex].value 
                    + " ] at ( " + this.tokens[this.currentTokenIndex].lineNum + ":" 
                    + this.tokens[this.currentTokenIndex].colNum + " ) - " + Production.Id + " ::== " + Production.Char);
                this.error = true;
                this.errors++;
            }
            return false;
        ...
        public parseBoolOp(production: Array<Production>): boolean {
            if (this.matchAndConsumeToken(TokenType.BoolOp, production, Production.BoolOp, false)) {
                ...
                this.log.push(DEBUG + " - " + PARSER + " - " + ERROR + ": " + ErrorType.BoolOpExpected + " - found [ " 
                    + this.tokens[this.currentTokenIndex].type + ", " + this.tokens[this.currentTokenIndex].value + " ] at ( " 
                    + this.tokens[this.currentTokenIndex].lineNum + ":" + this.tokens[this.currentTokenIndex].colNum + " ) - " 
                    + Production.Expr + " ::== == | !=");
                this.error = true;
         ...
         */
        public matchAndConsumeToken(token: TokenType, start: Array<Production>, rewrite: Production, expected: boolean): boolean {
            if (this.tokens[this.currentTokenIndex].type == token) {
                ...
                this.log.push(DEBUG + " - " + PARSER + " - " + "VALID - Expecting [ " + token + " ], found [ " 
                    ...
                    + " : " + this.tokens[this.currentTokenIndex].colNum + " )");
                // Add token to tree
                this.cst.addTNode(this.tokens[this.currentTokenIndex], this.tokens[this.currentTokenIndex].lineNum, 
                    this.tokens[this.currentTokenIndex].colNum);
                // consume token
                ...
                if (expected) {
                    this.log.push(DEBUG + " - " + PARSER + " - " + ERROR + ": " + ErrorType.TExpected + " [ " 
                        + Production.Expr + " ] " +  " - found [ " + this.tokens[this.currentTokenIndex].type + ", " 
                        + this.tokens[this.currentTokenIndex].value + " ] at ( " + this.tokens[this.currentTokenIndex].lineNum 
                        + ":" + this.tokens[this.currentTokenIndex].colNum + " )");
                    this.error = true;
                    this.errors++;
