				...
				if (!this.inComment && !this.foundQuote) {
					...
					*/
					...
					// Test for EOP
						...
						if (this.foundQuote) {
							this.errors.push(new Error(ErrorType.NO_END_QUOTE, sourceCode.charAt(this.endPtr - 1), this.startCommentLine, this.startCommentCol));
						}
					...
					*/
						...
						if (this.endPtr == sourceCode.length) {
							...
							if (rCOMMENTSTART.test(sourceCode.substring(this.startPtr, this.endPtr + 1))) {
								this.errors.push(new Error(ErrorType.NO_END_COMMENT, "*/", this.startCommentLine, this.startCommentCol));
							// Otherwise, any other character besides EOP is invalid
							} else {
								this.errors.push(new Error(ErrorType.INVALID_T, sourceCode.charAt(this.endPtr - 1), this.currentLineNum, this.currentColNum));
							}
							break;
						}
			...
			for (let i = 0; i < this.resultsArray.length; i++) {
				if (this.resultsArray[i].errors.length == 0) {
					...
					if (this.resultsArray[i].inComment) {
						this.resultsArray[i].errors.push(new Error(ErrorType.NO_END_COMMENT, "*/", this.startCommentLine, this.startCommentCol));
						...
						this.resultsArray[i].errors.push(new Error(ErrorType.NO_END_QUOTE, "\"", this.startQuoteLine, this.startQuoteCol));
						...
						this.resultsArray[i].warnings.push(new Warning(WarningType.NO_EOP, "$", this.currentLineNum, this.currentColNum));
					}
				}
			}
